...
export default function LiveAttendanceModal({
  ...
  // Target-specific attendance email notices (Only Absentees or Only Presentees)
  const [notifyTargetModal, setNotifyTargetModal] = useState<"ABSENT" | "PRESENT" | null>(null);
  const [isSendingNotice, setIsSendingNotice] = useState(false);
  const [notifySuccessMsg, setNotifySuccessMsg] = useState<string | null>(null);

  const handleSendAttendanceNotice = async (target: "ABSENT" | "PRESENT") => {
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setNotifySuccessMsg(data.message);
        setTimeout(() => {
          setNotifyTargetModal(null);
          setNotifySuccessMsg(null);
        }, 3500);
        fetchLiveAttendance(false);
        if (onAttendanceChanged) onAttendanceChanged();
      } else {
        alert(data.message || "Failed to dispatch emails.");
      }
    } catch (err) {
      console.error(err);
      alert("Error sending attendance notice.");
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-4 md:p-6 bg-black/85 backdrop-blur-md animate-in fade-in">
              ...
              <div className={`bg-white border border-slate-200 rounded-2xl sm:rounded-3xl p-4 sm:p-5 shadow-sm flex-1 flex flex-col ${mobileTab === "feed" ? "hidden lg:flex" : "flex"}`}>
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5 pb-3 border-b border-slate-100">
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto flex-wrap sm:flex-nowrap">
                    {/* Quick Email Absentees Button */}
                    <button
                      type="button"
                      onClick={() => setNotifyTargetModal("ABSENT")}
                      className="px-2.5 py-1.5 rounded-xl bg-rose-50 hover:bg-rose-100 border border-rose-200 text-rose-700 text-[11px] font-extrabold transition flex items-center gap-1.5 shadow-2xs cursor-pointer active:scale-95"
                      title="Send Absentee Account On-Hold email notice with WhatsApp appeal link"
                    >
                      <Mail className="w-3.5 h-3.5 text-rose-600" />
                      <span>Email Absentees ({stats.pendingCheckIn})</span>
                    </button>

                    {/* Quick Email Presentees Button */}
                    <button
                      type="button"
                      onClick={() => setNotifyTargetModal("PRESENT")}
                      className="px-2.5 py-1.5 rounded-xl bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 text-emerald-700 text-[11px] font-extrabold transition flex items-center gap-1.5 shadow-2xs cursor-pointer active:scale-95"
                      title="Send Attendance Verified / Duty Completed email confirmation"
                    >
                      <Mail className="w-3.5 h-3.5 text-emerald-600" />
                      <span>Email Presentees ({stats.markedPresent})</span>
                    </button>
      ...
      {/* Target-Specific Attendance Email Dispatch Modal (Only Absentees or Only Presentees) */}
      {notifyTargetModal && (
        <div className="fixed inset-0 z-60 bg-black/70 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 animate-in fade-in">
          <div className="bg-white rounded-3xl max-w-lg w-full p-5 sm:p-6 shadow-2xl border border-slate-200 space-y-4 animate-in zoom-in-95">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2.5">
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${
                  notifyTargetModal === "ABSENT" ? "bg-rose-100 text-rose-600" : "bg-emerald-100 text-emerald-600"
                }`}>
                  <Mail className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="text-sm sm:text-base font-extrabold text-slate-900">
                    {notifyTargetModal === "ABSENT" ? "Email Absentees (Hold Notice)" : "Email Presentees (Duty Verified)"}
                  </h3>
                  <p className="text-[11px] text-slate-500">
                    {notifyTargetModal === "ABSENT"
                      ? "Send Account On-Hold warning with direct WhatsApp appeal"
                      : "Send official duty completion confirmation"}
                  </p>
                </div>
              </div>
              <button
                ...
                onClick={() => setNotifyTargetModal(null)}
              ...
              <>
                  ...
                  <div className="space-y-3">
                    <div className="p-3.5 bg-rose-50 border border-rose-200 rounded-2xl text-xs space-y-2">
                      <div className="flex items-start gap-2 text-rose-900 font-bold">
                        ...
                        <span>Absentee Notification Package:</span>
                      </div>
                      <ul className="list-disc list-inside space-y-1 text-slate-700 text-[11px] pl-1">
                        <li>
                          Recipients: <strong>{roster.filter(c => c.attendanceStatus !== "PRESENT" && c.attendanceStatus !== "LATE").length} Absent Candidate(s)</strong>
                        </li>
                        <li>
                          Template: <strong>Account Placed On Hold (Red Alert Template)</strong>
                        </li>
                        <li>
                          Includes 1-tap <strong>WhatsApp Appeal Button (7986955634)</strong> for students with valid emergency reasons.
                        </li>
                        <li>
                          Sets candidate profile status to <strong>ON_HOLD</strong>.
                        </li>
                      </ul>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="p-3.5 bg-emerald-50 border border-emerald-200 rounded-2xl text-xs space-y-2">
                      <div className="flex items-start gap-2 text-emerald-900 font-bold">
                        ...
                        <span>Presentee Notification Package:</span>
                      </div>
                      <ul className="list-disc list-inside space-y-1 text-slate-700 text-[11px] pl-1">
                        <li>
                          Recipients: <strong>{roster.filter(c => c.attendanceStatus === "PRESENT" || c.attendanceStatus === "LATE").length} Present Candidate(s)</strong>
                        </li>
                        <li>
                          Template: <strong>Attendance Verified / Duty Completed (Green Confirmation)</strong>
                        </li>
                        <li>
                          Includes direct button to access Student Portal Dashboard and future shifts.
                        </li>
                      </ul>
                    </div>
                  </div>
                ...
                <div className="pt-2 flex items-center justify-end gap-2.5">
                  ...
                  <button
                    type="button"
                    disabled={isSendingNotice}
                    onClick={() => handleSendAttendanceNotice(notifyTargetModal)}
                    className={`px-5 py-2.5 text-white font-extrabold text-xs rounded-xl shadow-md transition disabled:opacity-50 flex items-center gap-1.5 cursor-pointer active:scale-95 ${
                      notifyTargetModal === "ABSENT"
                  ...
                  >
                    {isSendingNotice ? (
                      ...
                      <>
                        ...
                        <span>
                          {notifyTargetModal === "ABSENT"
                            ? `Send Hold Notice to Absentees`
                            : `Send Verified Notice to Presentees`}
                        </span>
                      </>
                    )}
                  </button>
