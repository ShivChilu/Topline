...
4. Output: Format candidate lists as clean, neat Markdown tables.`
    ...
    while (iterations < MAX_ITERATIONS) {
      ...
      if (responseMessage.tool_calls && responseMessage.tool_calls.length > 0) {
        ...
        // Continue loop so the model reads the tool output and writes out the final detailed answer
      } else {
        // No more tool calls: final answer generated by the model
    ...
    if (!finalReply) {
      try {
        const finalRes = await createCompletionWithFallback({
          messages: formattedMessages,
          temperature: 0.2,
          max_tokens: 2048,
        });
        ...
        console.warn("[AdminCopilot] Final summary generation error:", err);
      }
    }
