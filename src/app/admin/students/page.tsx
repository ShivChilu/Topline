    ...
    <div className="space-y-6 text-white">
      ...
      <div className="bg-[#0c0d12] p-4 rounded-xl border border-gray-800 flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded text-xs font-bold transition uppercase tracking-wider ${
              statusFilter === "active" ? "bg-emerald-500 text-black" : "bg-gray-800/60 text-gray-400 hover:text-white"
            }`}
          >
            Active Only
          </button>
      ...
      {loading ? (
        <div className="text-center py-12">
          <p className="text-gray-500">Loading student files...</p>
        </div>
      ) : students.length === 0 ? (
                ...
                {students.map((student) => (
                  <tr key={student._id} className="hover:bg-gray-800/10 transition">
                    ...
                    <td className="p-4 text-gray-400">
                      ...
                      <div className="text-xs text-gray-500 uppercase">{student.universityId}</div>
                    </td>
      ...
      {activeStudent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75">
          <div className="bg-[#0c0d12] border border-gray-800 rounded-xl p-6 max-w-md w-full space-y-4">
            ...
            <div className="space-y-2 text-sm text-gray-300">
              <p><span className="font-semibold text-gray-500">Phone:</span> {activeStudent.phone}</p>
              <p><span className="font-semibold text-gray-500">Email:</span> {activeStudent.email}</p>
              <p><span className="font-semibold text-gray-500">University:</span> {activeStudent.university}</p>
              <p><span className="font-semibold text-gray-500">Member Since:</span> {new Date(activeStudent.createdAt).toLocaleDateString("en-GB")}</p>
            </div>

            <button
              onClick={() => setActiveStudent(null)}
              className="w-full bg-gray-800 hover:bg-gray-700 text-white font-bold py-2 rounded text-sm transition"
            >
              Close Window
            </button>
          </div>
