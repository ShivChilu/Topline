...
export default function AdminEventDetailPage(props: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-6 text-slate-900 pb-20">
      ...
      {/* Live Real-Time Attendance & Scannable QR Code Hub Modal */}
      {isLiveAttendanceModalOpen && (
        <LiveAttendanceModal
          isOpen={isLiveAttendanceModalOpen}
          eventId={eventId}
          eventName={event?.name || "Event Attendance"}
      ...
      {/* WhatsApp Link & Automation Settings Modal */}
      {isWhatsappModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl border border-slate-100 overflow-hidden space-y-0">
            ...
            <div className="px-6 py-4.5 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl bg-[#25D366]/20 border border-[#25D366]/40 flex items-center justify-center text-[#25D366] shrink-0 shadow-inner">
                  <MessageSquare className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="text-base font-bold text-white">Event WhatsApp Group Link</h3>
            ...
            <div className="p-6 space-y-4 text-xs">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-300 focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 rounded-xl px-3.5 py-2.5 text-xs text-slate-900 font-medium placeholder:text-slate-400 focus:outline-none"
                  autoFocus
                />
                <p className="text-[11px] text-slate-500">
                  This link is automatically sent in selection emails and unlocked in candidate passes upon confirmation.
                </p>
              </div>

              {/* Action Buttons: Copy / Test Open if link exists */}
              {event?.whatsappGroupLink && (
                <div className="flex items-center gap-2 pt-1 flex-wrap">
                  <button
                    ...
                    className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition text-[11px] cursor-pointer"
                  >
                    {copiedWhatsappLink ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5" />}
                    <span>{copiedWhatsappLink ? "Copied" : "Copy Saved Link"}</span>
                  </button>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200 font-bold px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition text-[11px]"
                  >
              ...
              {/* Auto-selection toggle section inside modal */}
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-2.5">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <Sparkles className="w-4 h-4 text-emerald-600 shrink-0" />
                    <div>
                      <span className="font-bold text-slate-800 block">Auto-Send Selection Emails</span>
                      <span className={`text-[10.5px] font-bold ${event?.autoSendSelectionEmail !== false ? "text-emerald-600" : "text-slate-400"}`}>
                        {event?.autoSendSelectionEmail !== false ? "Status: Active" : "Status: Disabled"}
                      </span>
                    </div>
                  </div>
                  <button
                  ...
                  >
                    {event?.autoSendSelectionEmail !== false ? "Turn Off Auto-Mails" : "Turn On Auto-Mails"}
                  </button>
                </div>
                <p className="text-[11px] text-slate-500 leading-relaxed">
                  When enabled, candidates applying for this event immediately receive selection emails with a 2-hour confirmation window and WhatsApp group unlock.
                </p>
              </div>
            </div>
            ...
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-2.5">
              ...
              <button
                type="button"
                onClick={async () => {
                  await handleSaveWhatsappGroupLink();
                  setIsWhatsappModalOpen(false);
                }}
                disabled={savingWhatsappLink}
                className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-5 py-2 rounded-xl text-xs flex items-center gap-1.5 transition shadow-sm cursor-pointer disabled:opacity-50"
              >
                {savingWhatsappLink ? (
                  <>
                    <div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />
