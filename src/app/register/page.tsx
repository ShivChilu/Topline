...
function RegisterForm() {
  ...
  // Auto-detect referral code from query params e.g. /register?ref=SHIV25
  useEffect(() => {
  ...
  return (
    <div className="w-full max-w-lg bg-white rounded-3xl p-8 sm:p-10 border border-slate-200 shadow-xl space-y-6">
      ...
      {formData.referralCode && (
        <div className="bg-gradient-to-r from-red-50 via-rose-50 to-amber-50 border border-red-200 text-red-800 px-4 py-3 rounded-2xl text-xs sm:text-sm flex items-center justify-between shadow-xs">
          <div className="flex items-center space-x-2">
            <Gift className="w-4 h-4 text-red-600 flex-shrink-0" />
            <span className="font-bold">
              Friend Invite Code <span className="underline font-mono uppercase font-black">{formData.referralCode}</span> applied!
            </span>
          </div>
      ...
      <form onSubmit={handleRegister} className="space-y-4">
          ...
          {(showReferralInput || formData.referralCode) && (
            <div className="relative animate-in fade-in">
              ...
              <input
                ...
                onChange={handleChange}
                placeholder="e.g. SHIV25"
                className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 text-sm text-slate-900 uppercase font-mono tracking-wider focus:outline-none focus:border-red-600 focus:ring-2 focus:ring-red-600/20 transition"
              />
            </div>
          )}
        </div>
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-[#ED0000] hover:bg-[#C00000] disabled:opacity-50 text-white font-extrabold py-3.5 rounded-2xl uppercase tracking-wider text-xs sm:text-sm transition shadow-md hover:shadow-lg flex items-center justify-center space-x-2 mt-4 cursor-pointer"
        >
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#f8fafc] text-slate-700 relative grid-bg overflow-hidden">
      ...
      <main className="flex-grow flex items-center justify-center px-4 py-16 relative z-10">
        <Suspense
          fallback={
            <div className="w-full max-w-lg bg-white rounded-3xl p-12 border border-slate-200 shadow-xl text-center space-y-3">
              <div className="w-8 h-8 border-4 border-red-600 border-t-transparent rounded-full animate-spin mx-auto"></div>
              <p className="text-xs font-bold text-slate-500">Loading student registration...</p>
            </div>
          }
        >
          <RegisterForm />
