...
export default function HowItWorksPage() {
  const steps = [
    ...
    {
      icon: <UserCheck className="w-10 h-10 text-rose-600" />,
    ...
    {
      icon: <CalendarCheck className="w-10 h-10 text-rose-600" />,
    ...
    {
      icon: <Award className="w-10 h-10 text-rose-600" />,
      ...
      <main className="flex-grow max-w-5xl mx-auto px-4 py-16 w-full">
        ...
        <div className="space-y-12">
          {steps.map((step, idx) => (
            <div
              key={idx}
              className="flex flex-col md:flex-row items-center gap-6 bg-[#0c0d12] p-8 rounded-xl border border-gray-800 hover:border-rose-600/50 transition duration-300"
            >
